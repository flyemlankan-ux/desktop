...
### 2026-09-19 — retest loop: cleanup, motion, accessibility and real SSH

- Native `performance156-fixed`: all six jobs AND records cleaned in0.312s after bounded unknown-state re-probe fix. Real-tmux focused regression passed60jobs/10rounds, including five actual final-server-exit races. Unknown errors are not interpreted as absence; permanent uncertainty retains deletion intent.
- Performance remains open: six real terminal tabs ready1.59s, total idle CPU6.48% of one core; ~2.3MiB burst12.29s FAILED initial10s budget (earlier6.22s); heartbeat p95~60ms/max80ms stayed responsive. No budget loosened. Measurement breakdown and stock comparison next.
- `keyboard156-motionfixed-v2`:10 native checks PASS including real shell block/bar/underline blink requests rendered steady with reduced motion. Test corrected short-output marker search (trailing blank rows made last8-line search invalid). Actual parser/CSS negative-control proof18pass separately.
- `accessibility156-v2`:4 native checks PASS. Actual browser accessibility service activated after terminal creation; real xterm accessibility rows and Firefox accessible objects contain shell output/labels, exact job removed on close. First test incorrectly assumed stable browser-global observer count; now diagnostic only, exact own lifecycle covered separately. Real VoiceOver speech/IME remain unproved.
- `ssh156-key`:4 native checks PASS against authorized Chubs using existing key path (never copied), strict existing host key, no forwarding. Remote/local ordered rows and real remote exit23 stop later local action. Commands only printed markers; no remote files/paid AI calls. Existing-agent-only first attempt did not authenticate; retained failure.
- Native CGEvent drag first attempt produced no DOM drag events. Verified own geometry but no pass; window-annotated PID-only experiment next. No global mouse fallback/original browser manipulation.
- Shortened native setup help without hiding warnings;15613tests and strictpatch checks pass; updated visual proof pending.
- Whole-image privacy scanner now rejects profile artifacts outside app and inside resource archives;11tests pass. Clipboard preservation cannot guarantee request-only/lazy formats or atomic restore on personal desktop; helper fails closed,12synthetic tests only. Real clipboard proof needs disposable login session.
- Scope unchanged: actual Zen with terminal tabs. Not final compiled release, personal migration or public notarization. Next handoffs: high reasoning for lifecycle/input/security, medium for routine replay; native journey plus focused regressions and explicit failed/not-run evidence.

### 2026-09-19 — bounded background output fixed and repeated

- Three detailed pre-fix runs measured~0.12s producer completion but hidden xterm parsing once per second; total9.36/11.30/12.36s, two failed10s budget. Selected viewer finished~0.2–0.3s. Timing evidence isolates hidden-document scheduling, not slow commands. Stock156 isolated idle comparisons retained (one exceeded provisional25%CPU by0.31point); these are local smoke measurements, not release-performance marketing.
- Added bounded public-write pipeline:256KiB decoded payload +64 writes maximum per pipe,64KiB+8 decoder reserve, every callback retained, EOF/error drain, current-read-only cancellation. No global timer change, vendor patch, proposed xterm API or unbounded pending promise chain. Focused source tests include10,000 reads, streaming prefix and unload.
- Three full native post-fix runs all pass:2.3MiB across6real jobs in4.15/3.21/4.22s, worst main-thread heartbeat88/93/80ms, cleanup0.18/0.17/0.18s. Logs/results performance156-pipeline-{1,2,3}. Final compiled build still must repeat acceptance.
- Native private-only test passes actual refusal message AND enabled OK interaction; no tab/job created. Folder chooser remains independent/open.
- Real155->156 synthetic migration passes two profiles: actual encrypted dummy login decryption, cookie, bookmark/history, container/workspace/preferences and restored tab, source127file hashes unchanged. This is not personal-account proof.
- Gecko-owned native pointer path now delivers real trusted dragstart/drop/dragend and desired reorder. Tests needed correction for capture-phase MIME timing and post-pointer redundant index-move; full corrected journey replay still pending. Prior failed traces retained.
- App-only delivery helper has22synthetic tests, exact identity/assets/signature checks, staged publication, retained old app, same-engine olderBuildID refusal and explicit rollback risk. No actual /Applications operation yet. Whole-DMG privacy11tests; clipboard12synthetic fail-closed tests. No new delivery claim.

### 2026-09-19 — native retests expose folder Undo and search layout gaps

- Actual mixed-folder lifecycle: native rename, workspace rename, recursive Delete and exact two-job cleanup pass; neighboring same-setup job survives. Actual Cmd+Shift+T restores both ownership receipts; selecting lazy tabs shows ended/Start again without silently rerunning commands. Nested Zen folder structure FAILS. Web-only control reproduces the same native Undo structure failure; product repair remains required, not waived as upstream.
- Fast Unicode search composition exposed a real first-result layout bug: result text added a row, resizing the terminal and clearing selection. CSS now reserves that result row. Seven rendered checks pass, including a negative control that restores old CSS and reproduces resize/lost selection. Parent real-browser input156-v3 passes five checks: exact Unicode PTY bytes, cancellation, Meta escape sequence, composing Enter isolation, and selection inside search. Browser composition is not a claim about actual macOS input-source UI.
- Added the rendered search-layout check to the cloud build workflow. Current cloud builds predate this CSS change and cannot prove its final compiled delivery.
- Native share-import initial run failed because the actual Add button was obscured by Zen's update-animation border. Retained failure; readiness diagnosis underway, no forced click or page deletion.
- Main26/saved9/grouping10/project-method11 native passes remain bounded evidence. Actual pointer repeatability, chooser, final compiled package and personal migration remain open. Original Zen and personal profiles untouched.
- Handoff: high reasoning for folder recovery/security/input, focused source tests plus real native replay and negative controls. Scope stays actual Zen plus terminals, not a terminal-only demo; no completion claim.

- Follow-up: input156-v4 and v5 also pass all five scoped checks, giving three consecutive native passes. Expanded rendered search test now13checks, including240px width and clearing/re-entering query. Committed/pushed search repair4024639; existing cloud snapshots do not contain it.
- Share retest waits the genuine temporary startup animation and button hit-test; no forced click/DOM removal. share-import156-v2 passes7checks. Extended native run passes10checks including actual persisted terminal-key rejection, real preview cancellation with no mutation, and rapid duplicate activation creating exactly one workspace with genuinely nested folder/web tabs. Uses supported loopback server preference, unmodified production client/manager/importer; external sharing authentication/upload remains not_run.
- Independent folder recovery review identified collision, placeholder, sibling-order, empty-root and missing-workspace edges before acceptance. Repair remains in progress. Added pinned official156 SessionStore source receipt and strict engine-resource verification (11tests pass); installer helper22 and packaging4/privacy11 focused regressions pass. These are not folder-repair native passes.

### 2026-09-19 — nested recovery repaired, edge failures closed with native proof

- Closed folders now retain their own nested structure, names/icons, workspace and sibling anchors. Collision remapping occurs before creating restored groups; existing folders and terminal ownership records stay unchanged. Empty folder close records use normal Undo without fake tabs/jobs, respect history limits/clearing, and survive browser restart.
- Native retests exposed two additional real failures: child-last closure selected a nonexistent child transaction; deleting a workspace with no remaining real tabs reset the correct last-group marker. Root close-history ownership is now separate from child group identity. A two-line return after Zen's existing empty-tab filter makes no-op removeTabs requests truly no-op. Ordinary and mixed real-tab close paths remain unchanged.
- Parent exact official156 overlay includes both engine modules, with zero-fuzz complete patches and exact old-baseline equality before replacement. Registered ZenFolder/ZenFolders modules also verified. Asset verifier11tests includes rejecting stale engine modules. This remains an overlay, not a compiled release.
- `web-folders156-noop-standard` and `web-folders156-noop-childlast-v2`:11 native stages each PASS, including actual Delete/Undo, empty nested folders, first position, same-ID existing-folder preservation, removed workspace fallback, real quit/relaunch, history-clear notification, and disabled-history preference. Clear/disable are native service/preference methods, not Settings UI proof. Initial child-last replay closed synthetic pages before loading; corrected test waits actual data URI/title before flushing, preserving failed evidence.
- `folders156-noop-final`:8PASS, actual rename/Delete/Undo/Start again and method-level workspace deletion. Closed jobs never silently restart; exactly one explicit restart runs, neighbor survives. Earlier button failures were wrong WebDriver document contexts, corrected using the actual terminal content handle.
- `folder156-main`:26PASS before final no-op guard, including keyboard/editor/CLI-version-only, duplicate/reload, lazy restore, quit/crash, exact final close. Final package must repeat key journeys.
- Actual pointer readiness now waits trusted destination dragover with real tab type and accepted move before release; bounded timeout still releases and fails. Unsupported AXRaise accepted only after verifying owned foreground/focused window. `pointer156-ready`, `-v2`, `-v3`:12PASS each, real trusted drop/end/order plus native-method split and mirrored-window job invariants. Actual pointer drag-to-split and final compiled identity remain unproved.
