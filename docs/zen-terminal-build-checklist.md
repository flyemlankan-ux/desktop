...
| ZT-RECIPE | C07 C08 | ordered startup recipes | `todo` | planned | Targeted unit/process tests; Playwright rendered display and native packaged browser interaction proof | automated | no | no | `none` | `derived from index` |
| ZT-UI | C01 C02 | native containers and tab behavior | `todo` | planned | Targeted unit/process tests; Playwright rendered display and native packaged browser interaction proof | automated | no | no | `none` | `derived from index` |
