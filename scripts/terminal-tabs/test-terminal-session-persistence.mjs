...
let saved = 0;
let timerDelay;
let profilePath = "/synthetic/terminal-profile-a";
globalThis.Ci = { nsIFile: {} };
globalThis.Services = {
  appinfo: { OS: "Darwin" },
  dirsvc: {
    get(name) {
      assert.equal(name, "ProfD");
...
test("process sockets are stable per physical profile and distinct from copied profiles", () => {
  const first = manager.terminalSocketForProfilePath("/synthetic/one");
  assert.equal(first, manager.terminalSocketForProfilePath("/synthetic/one"));
  assert.notEqual(
    first,
    manager.terminalSocketForProfilePath("/synthetic/two"),
  );
  assert.match(first, /^zen-terminal-p[0-9a-f]{16}$/);
  assert.notEqual(
    first,
    "zen-terminal",
    "legacy shared sockets are never adopted",
  );
  assert.throws(
    () => manager.terminalSocketForProfilePath("relative"),
    /profile directory/,
  );
  assert.equal(
    manager.getTerminalTmuxSocket(),
    manager.terminalSocketForProfilePath(profilePath),
  );
});
...
async function realTmuxTest() {
  ...
  const socketPrefix = `zt-proof-${process.pid}-${Date.now()}`;
  const usedSockets = new Set();
  const testSocket = (original) => {
    assert.match(original, /^zen-terminal-p[0-9a-f]{16}$/);
    const isolated = `${socketPrefix}-${original.slice(-16)}`;
    usedSockets.add(isolated);
    return isolated;
  };
  ...
  const direct = (...args) =>
    spawnSync(
      ...
      [
        "-L",
        testSocket(manager.getTerminalTmuxSocket()),
        "-f",
        "/dev/null",
        ...args,
      ],
  ...
  handler = (options) => {
    ...
    const args = [...options.arguments];
    assert.deepEqual(args.slice(0, 2), ["-L", manager.getTerminalTmuxSocket()]);
    args[1] = testSocket(args[1]); // Always isolate the real test from every production session.
  ...
  try {
    ...
    // A byte-for-byte copied profile retains the tab ID and saved records,
    // but its physical profile directory must give it a different live shell.
    const sourceProfilePath = profilePath;
    const copiedPrefs = new Map(prefs);
    profilePath = "/synthetic/terminal-profile-b";
    assert.equal(
    ...
    assert.match(copyPid, /^\d+$/);
    assert.notEqual(
      copyPid,
      pid,
      "the copied profile must not attach to the source process",
    );
    assert.equal(
    ...
    prefs.clear();
    for (const [key, value] of copiedPrefs) prefs.set(key, value);
    assert.equal(
      await manager.terminalTmuxSessionState(tmux, "real-target"),
      "present",
    );
    assert.equal(
      ...
      pid,
      "deleting the copied profile's identical ID must leave the original process alive",
    );
    assert.equal(
      await manager.destroyTerminalSession("real-target", {
        tmuxCommand: tmux,
    ...
    for (const socket of usedSockets) {
      assert.ok(socket.startsWith(socketPrefix + "-"));
      spawnSync(tmux, ["-L", socket, "kill-server"]);
    } // Unique test sockets only; never a production or legacy socket.
    rmSync(directory, { recursive: true, force: true });
  }
}
test(
  "real module creates once, retains and resizes its process; copied profiles cannot attach or delete across profiles",
  realTmuxTest,
);

for (const [name, run] of tests) {
  prefs.clear();
  profilePath = "/synthetic/terminal-profile-a";
  calls = [];
  saved = 0;
  await run();
  console.log(`✓ ${name}`);
