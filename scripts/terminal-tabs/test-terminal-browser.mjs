...
function mapArgs(args = []) {
  return args.map((value, i) => {
    if (!i || args[i - 1] !== "-L") return value;
    assert.match(
      value,
      /^zen-terminal-p[0-9a-f]{16}$/,
      "Production socket must be profile-specific",
    );
    if (!testSockets.has(value))
      testSockets.set(
        value,
        testSockets.size ? `${socket}-${testSockets.size + 1}` : socket,
      );
    return testSockets.get(value);
  });
}
...
try {
      ...
      window.Services = {
        ...
        dirsvc: {
          get: (key) =>
            key === "ProfD"
              ? {
                  path: `${home}/profile${new URLSearchParams(location.search).get("profile") === "copy" ? "-copy" : ""}`,
                }
              : {
                  parent: {
                    path: helper,
  ...
  assert.match(panePid, /^[1-9][0-9]*$/, "Real shell PID must be nonempty");
  await check(
  ...
  await check(
    "Copied profile uses its own tmux shell even with the same saved session id",
    async () => {
      ...
      assert.equal(testSockets.size, 2);
      const copySocket = [...testSockets.values()][1];
      const copyPid = execFileSync(
        tmux,
        [
          "-L",
          copySocket,
      ...
      assert.match(
        copyPid,
        /^[1-9][0-9]*$/,
        "Copied shell PID must be nonempty",
      );
      assert.notEqual(
        copyPid,
        panePid,
        "A copied profile must never attach to the original shell",
      );
      assert.equal(
        tmuxQuery(
          "display-message",
  ...
  for (const { child } of processes.values())
    if (child.exitCode === null && child.signalCode === null)
      child.kill("SIGTERM");
  for (const testSocket of new Set([socket, ...testSockets.values()]))
    try {
      execFileSync(tmux, ["-L", testSocket, "kill-server"], {
        env: environment,
        stdio: "ignore",
      });
    } catch {}
