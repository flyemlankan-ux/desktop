...
log=open(run/'gecko.log','w');process=None;m=None;results=[];test_sessions=[]
...
def tmux(*args):return subprocess.run(['/opt/homebrew/bin/tmux','-L','zen-terminal',*args],capture_output=True,text=True,timeout=5)
def screen(name):
...
try:
 ...
 m.find_element('css selector','#zen-terminal-recipe-steps input').send_keys('printf started >> "$HOME/recipe-count"')
